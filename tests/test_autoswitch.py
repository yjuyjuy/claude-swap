...
class TestPerWindowThresholds:
    """--5h-threshold / --7d-threshold overrides and consume-first 5h-only
    landing with 7d deprioritization, below the threshold. The
    every-account-above-threshold regime is owned by upstream's
    soonest-recovery ranking (see TestEveryAccountAboveThreshold), so the
    fork's old 99% wedge-breaker escape was dropped in the sync."""
    ...
    def test_best_still_rejects_7d_heavy_landing(self, temp_home):
        # strategy=best: the max-fold landing gate is unchanged, so a peer with
        # an idle 5h but a near-maxed 7d is rejected and best holds. (Under
        # consume-first the 5h-only landing would still take it below the
        # threshold; here every account is above it, so upstream's
        # soonest-recovery ranking finds no qualifying candidate.)
